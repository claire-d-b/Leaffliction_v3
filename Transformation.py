...
from Transformation_types import (get_hls, get_hsv, get_gaussian_blur,
                                  ...
                                  get_median_blurring_small_noise, get_morphological_gradient, get_sobel, get_laplacian_operator, get_canny_edge, get_lab, get_gray, get_yuv)
from Histogram import calc_hist, get_additional_values, plot_multiple_images_histogram
from stats import get_len

def process_file(src: str, dst: str, category: str, augmented: bool) -> ndarray | None:
    ...
    if dst is None:
        ...
        plot_multiple_images_histogram(files, src, dst, ndst, category, augmented)

        return img


def process_input_augmentation(src=None, dst=None, option=None) -> None:
    transformations = "lab", "hsv", "morphological_gradient", "bilateral_filter", \
                      "median_blur_small_noise", "canny_edge"
    true_subdir = "Transformed"
    ...
    if src and path.isfile(src):
        ...
        img = process_file(src, dst=ndst, category="Transformed", augmented=False)
        if option:
        ...
        pattern = f"{folder}Base/*.JPG"  # You can use *.png or *.* to match more
        img = glob(pattern)
        ...
        if img:
            ...
            for i, image in enumerate(img):
                process_file(image, dst=dst, category="Transformed", augmented=False)
                ...
                if option:
                    for i, ttype in enumerate(transformations):
                        match option:
                            ...
                            case 3:
                                return get_median_blurring_large_noise(image, dst)
                            case 4:
