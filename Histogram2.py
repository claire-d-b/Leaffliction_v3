...
def plot_multiple_images_histogram(image_files, src, dst, ndst, ncategory,
                                   augmented, script=True):
    ...
    if valid_images != 0:
        ...
        if ncategory == "Transformed" or ncategory == "Augmented":
            print("ICIII")
            print(src)
            nsrc = path.basename(src)
            dst = f"{Path(src).parent}/{ncategory}/*.JPG"
            print(dst)

            modifications = ("lab", "hsv", "morphological_gradient",
                            "bilateral_filter", "median_blur_small_noise",
                            "canny_edge") if ncategory == "Transformed" else \
                            ("contrast", "scale_zoom",
            ...
            for m in modifications:
                ...
                print("dstname")
                print(src)
                print(f"{Path(Path(src).name).stem}_{Path(src).parent.parent.name}")
#                 print(f"{Path(dstname).name.split('_')[0]}_\
# {Path(dstname).parts[1]}")
                subname = f"{Path(Path(src).name).stem}_{Path(src).parent.parent.name}"
                print("subname")
                print(subname)
                category = Path(Path(src).name).stem
...
features.csv", mode='w')
            ...
            # pattern = f"{Path(argv[1]).parent}/*.csv" if not argv[1].endswith("/") else f"{argv[1]}*.csv"
            pattern = f"{argv[1]}/*.csv" if not argv[1].endswith('/') else f"{argv[1]}*.csv"
            print("patternnn")
            print(pattern)
            # print("hehehhe")
            # print(pattern)
