...
#     imwrite(f"{dst}/{subdir}/{filename}_zoomed_cropped.JPG", img_zoomed_cropped)
...
def get_horizontal_flip(src: str, dst: str):
    ...
    horizontal_flip_rgb = cvtColor(img_horizontal_flip, COLOR_BGR2RGB)
    imwrite(f"{dst}/{subdir}/{filename}_horizontal_flip.JPG", img_horizontal_flip)
...
def get_perspective_transformation(src: str, dst: str) -> None:
    ...
    pts1 = float32([[cols*1/12,rows*1/12],[cols*11/12,rows*1/12],[cols*11/12,rows*11/12],[cols*1/12,rows*11/12]])  # HG,HD,BD,BG
    ...
    imwrite(f"{dst}/{subdir}/{filename}_perspective_transformation.JPG", image)
