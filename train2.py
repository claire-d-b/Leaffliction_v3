...
def train():
    ...
    if len(argv) != 2:
        ...
        print("combined df", combined_df.values)

        combined_df.to_csv("features.csv")
        claire_df = combined_df.groupby(["Subname", "Name", "Category", "Modification"], as_index=False).median(numeric_only=True)
        ...
        df = normalize_df(df)
        # ppdf = df.copy()
        # ndf = df.groupby("Category", as_index=False).sum(numeric_only=True)
        print("summed_df")
        print(df)
        ...
        for i in range(len(categories)):
            ...
            overall_values = [float(item) for sublist in df[df
                              ['Category'] == categories[i]].iloc[:, 2:].values
                              for item in sublist]
            # print("overall")
        ...
        ppdf = df.groupby(["Subname", "Category"], as_index=False).median(numeric_only=True)
        ppdf = normalize_df(ppdf)
        print(claire_df.shape)
        print("ppdf", claire_df)
        # ppdf = ppdf.reset_index()
        pairplot(claire_df, hue='Category', palette=[random_colors
                                                [i]
                                                for i in range(len
                                                               (categories))],
